...
public class LinkedList<T> implements ListInterface<T> {
    ...
    /**
     * This implementation uses getNodeAt() at each step, and is therefore
     * O(n^2). You should reimplement this method in O(n) time, without using
     * getNodeAt() at each step (Hint: build backwards).
     */
    public void reverse() {
        // Find the last node; this is the new head
        Node newHead = getNodeAt(size - 1);

        // Build the chain from (new) head to (new) tail
        Node current = newHead;
        for (int i = size - 2; i >= 0; i--) {
            // Find the last remaining node in original chain
            Node lastRemainingNode = getNodeAt(i);
            // This is the next node in the new chain
            current.next = lastRemainingNode;
            // Iterate current for next iteration
            current = current.next;
        }
        // Make sure the new tail node points to null
        current.next = null;
        // Save the new chain
        head = newHead;
    }
